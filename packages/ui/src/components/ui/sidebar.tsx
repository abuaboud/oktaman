...
>(
  (
    ...
    // Read sidebar state from cookie on mount, fallback to defaultOpen
...
>(({ className, onClick, ...props }, ref) => {
  ...
  const openIconRef = React.useRef<any>(null)
    ...
    <Button
      ...
      onMouseEnter={() => {
        if (state === "expanded") {
          ...
          openIconRef.current?.startAnimation()
        }
      }}
      onMouseLeave={() => {
        if (state === "expanded") {
          ...
          openIconRef.current?.stopAnimation()
        }
      }}
      {...props}
    >
      {state === "expanded" ? (
        <PanelLeftCloseIcon ref={closeIconRef} size={16} />
      ) : (
        <PanelLeftOpenIcon ref={openIconRef} size={16} />
      )}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-8 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      ...
      collapsedPadding: {
        default: "group-data-[collapsible=icon]:!p-2",
        none: "group-data-[collapsible=icon]:!p-0",
      },
    },
    defaultVariants: {
      variant: "default",
      size: "default",
      collapsedPadding: "default",
    },
  }
)
  ...
  (
    ...
    const button = (
      <Comp
        ...
        className={cn(sidebarMenuButtonVariants({ variant, size, collapsedPadding }), className)}
        onClick={onClick}
        {...props}
      />
    )
