...
interface TelegramChannelFormProps {
  initialConfig?: TelegramChannelConfig;
  onSave: (name: string, config: TelegramChannelConfig) => Promise<void>;
  onCancel: () => void;
  hideActions?: boolean;
}

export function TelegramChannelForm({ initialConfig, onSave, onCancel, hideActions }: TelegramChannelFormProps) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        // Extract error message from OktaManError response or use fallback
